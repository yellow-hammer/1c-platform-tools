...
Расширение поддерживает работу с AI-агентами (Cursor, Claude, Copilot) двумя способами.

### Навык для агента (SKILL.md)

Команда **«Добавить»** в группе **«Навыки для AI»** копирует в проект файл с инструкциями: агент узнаёт, какие команды расширения вызывать (загрузка конфигурации, запуск Конфигуратора и т.д.), и предлагает выполнить их из палитры или панели «Инструменты 1С». Выберите папку установки (для Cursor — `.cursor/skills/1c-platform-tools`, для Copilot — `.github/copilot/skills/1c-platform-tools`, для Claude — `.claude/skills/1c-platform-tools`).
...
### MCP-сервер (Cursor и др.)

В состав расширения входит MCP-сервер, позволяющий агенту вызывать команды через Model Context Protocol (инструменты `run_designer`, `run_enterprise`, `load_configuration`, `dump_configuration`, а также `run_command` с произвольным `commandId`).

**Настройка MCP в Cursor:**

1. Узнайте путь к расширению: в Cursor откройте список расширений, найдите «1C: Platform tools» и скопируйте путь к папке расширения (или через «О расширении»).
2. Откройте файл настроек MCP (в Cursor: настройки MCP или файл `%APPDATA%\Cursor\User\globalStorage\cursor.mcp\mcp.json` на Windows).
3. Добавьте сервер (подставьте свой путь к расширению и при необходимости версию):
...
```json
{
  "mcpServers": {
    "1c-platform-tools": {
      "command": "node",
      "args": [
        "C:/Users/<user>/.cursor/extensions/yellow-hammer.1c-platform-tools-<version>/resources/mcp-server/index.mjs",
        "${workspaceFolder}"
      ]
    }
  }
...
4. Перезапустите Cursor или перезагрузите окно. Подробнее см. `resources/mcp-server/README.md` в составе расширения.
